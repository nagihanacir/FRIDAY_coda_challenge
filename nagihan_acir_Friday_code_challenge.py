...
def split_address(address):
    ...
    ##find the indexes of tokens contain numbers and put them in a list named indexes
    ...
    ## if first token is a number, we can think it as a housenumber, and rest of it as the street name
    if indexes[0] == 0 :
        converted_address["street"] =   (" ".join(address_tokens[1:]))   
        converted_address["housenumber"] = address_tokens[0]

    ## if first token is not a number, and all address contains only 1 token with number >>
    #  we can extract after the number as housenumber, rest of it as the street name
    ...
    ## if address contains more than 1 tokens with number, we can extract street name until the token which contains first number, rest of it as housenumber
